...
save(metadata, file = "data/metadata/metadata.Rda")
```
...
write_tsv(BFC10, file = "data/filtering_data/bfc10.txt", col_names = FALSE)
...
# Save the list of bad samples as a .txt file without column names
write_tsv(admixed_individuals, file = "data/filtering_data/admixed_individuals.txt", col_names = FALSE)
...
##### 1a: Generate the header for a general bash shell script to run SNPHYLO:
...
##### Command line code for running SNPHYLO. UNCOMMENT this code in the shell script and run: `sbatch snphylo.sh`
```{r}
