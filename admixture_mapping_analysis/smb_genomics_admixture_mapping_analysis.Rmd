...
In this step, we are using the fully filtered, non-thinned VCF dataset `popgen.vcf` generated in Analysis 2 (SNP Filtering...) and generating input files for MIXMAPPER. 
...
```{bash}
#plink --vcf ../../data/processed_vcf/popgen.vcf --recode --recode-INFO-all --out ../../data/processed_ped/popgen
```
...
In this step, we need to modify the `popgen.map` file generated in Step 4a above, because the software program EIGENSOFT expects to see chromsome and SNP designations in a particular format. The software was built to analyze data sequenced from humans and aligned to the human genome. Thus, the "chromosome" column (column 1) of the .map file will only recognize autosomal chrosomosome designations (1-22). Column 2 of the .map file contains SNP identifier information, and this can be in any format.
...
##### 4b.1.2. Generate copies of each of the other necessary input file type (.ped) to `popgen_mod.ped` to correspond.

##### 4b.2. Generate par.PED.EIGENSTRAT file as input for EIGENSOFT
...
###### 4b.2.1. Navigate into the `data/batch_cmd_lists/` folder and copy and paste the code below in a new nano .txt file:
...
##### 4b.3. Generate a general bash shell script header for all EIGENSOFT commands
...
###### 4b.3.1: Copy and paste the code below in a new shell script file:
...
##### 4b.4. Convert PED files to EIGENSTRAT format.

###### 4b.4.1. Copy and paste the code below in the shell file generated in Step 4b.1.1 above, which will generate .eigenstratgeno, .snp, and .ind files for the full dataset:
