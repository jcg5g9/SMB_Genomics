...
Make sure to copy the `popgen_spb_hybrid.vcf` dataset generated in Analysis 2 into a new sub-directory of the `data/` folder in this directory: `data/processed_vcf`
...
```{bash}
#plink --vcf ../../data/processed_vcf/popgen_spb_hybrid.vcf --recode --recode-INFO-all --out ../../data/processed_ped/popgen
```
...
ind_file <- ind_file %>%
  separate(sample_id, into = c("sample_id","u","pop"), sep = " ", remove = T)
