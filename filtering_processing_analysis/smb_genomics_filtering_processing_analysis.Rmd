...
Danecek, P., Auton, A., Abecasis, G., Albers, C. A., Banks, E., DePristo, M. A., … Durbin, R. (2011). The variant call format and VCFtools. Bioinformatics, 27, 2156–2158. doi:10.1093/bioinformatics/btr330

### STEP 1: Generate a general bash shell script for all VCF commands header with which to run all VCF commands
...
#### 1a: Generate the header for a general bash shell script to run VCFTOOLS:
...
### STEP 2: Filter out individual samples with greater than 20% missing data across all SNPs.
...
#### 2a: Copy and paste the code below in the shell file generated in Step 1a above, which will generate a file with missing genotype call rate information for all samples:

##### Command line code for obtaining missing genotype call rate information. UNCOMMENT this code in the shell script and run: `sbatch vcftools.sh`
```{bash}
...
write_tsv(smb_genomics_badsamples, file = "data/filtering_data/smb_genomics_badsamples.txt", col_names = FALSE)
```

### STEP 2: Filter SNPs for a minimum depth of coverage of 15X
In this step, we are filtering the dataset to retain only SNPs that have a minimum depth of coverage across all individuals of 15X. 

#### 2a: Include the code below in the vctools.sh shell script to filter for minimum depth of coverage:

##### Command line code for filtering SNPs at minimum depth of coverage. UNCOMMENT this code in the shell script.
```{bash}
...
### STEP 3: Filter individuals from the dataset with greater than 20% missing data across SNPS.
In this step, we are omitting any individual samples which do not have genotypes at greater than 20% of SNP loci. We first run command line code in VCFTools which prints a list of all individuals and their associated genotype call rates across loci. We then run an additional command line code which eliminates individuals greater than the 20% threshold from the filtered dataset.

#### 3a: Include the code below to generate a file with missing genotype call rate information for all samples:
...
This code generates the file: `data/filtering_data/smb_genomics_missing_genotype.imiss`. This file should be used to generate a list of sample IDs with missing genotype call rates greater than 20%.

#### 3b: Generate list of sample IDs for individuals with greater than 20% missing data; run the Rmd chunk below:

##### Generate list of sample IDs:
```{r}
# read in missing genotype file ('.imiss')
genotypes_missing <- read_tsv("data/filtering_data/smb_genomics_missing_genotype_again.imiss")

# turn file into data frame and mutate
genotypes_missing <- genotypes_missing %>%
  as.data.frame() %>%
  mutate(INDV = factor(INDV))

indivs_missing <- genotypes_missing %>%
  filter(F_MISS > 0.2)
```
