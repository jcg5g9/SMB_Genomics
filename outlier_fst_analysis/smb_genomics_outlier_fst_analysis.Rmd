...
midark_white_elk_pops <- smb_pops_noBFC10 %>%
   filter(finerad_pops == "WHITE" |finerad_pops == "MIDARK"|finerad_pops == "ELK")

midark_white_bayou_pops <- smb_pops_noBFC10 %>%
   filter(finerad_pops == "WHITE" |finerad_pops == "MIDARK"|finerad_pops == "BAYOU")

midark_white_uppark_pops <- smb_pops_noBFC10 %>%
   filter(finerad_pops == "WHITE" |finerad_pops == "MIDARK"|finerad_pops == "UPPARK")

midark_skia_illi_pops <- smb_pops_noBFC10 %>%
   filter(finerad_pops == "SKIA" |finerad_pops == "MIDARK"|finerad_pops == "ILLI")
...
midark_white_elk_pops <- cbind(midark_white_elk_pops$finerad_pops)
as.vector(midark_white_elk_pops)

midark_white_bayou_pops <- cbind(midark_white_bayou_pops$finerad_pops)
as.vector(midark_white_bayou_pops)

midark_white_uppark_pops <- cbind(midark_white_uppark_pops$finerad_pops)
as.vector(midark_white_uppark_pops)

midark_skia_illi_pops <- cbind(midark_skia_illi_pops$finerad_pops)
as.vector(midark_skia_illi_pops)
...
#Now, I am running cross validation on my samples, using 90 % of samples as a training set and 10% of samples as a test set, and choosing the number of PCs per analysis that maximizes successful assignment

#SMB outlier SNPs only
xval_smb_noBFC10_outliers_genind <- xvalDapc(smb_noBFC10_outliers_genind, smb_pops, n.pca.max = 300, training.set = 0.9,result = "groupMean", center = TRUE, scale = FALSE, n.pca = NULL, n.rep = 30, xval.plot = TRUE)

xval_smb_noBFC10_outliers_genind_df <- as.data.frame(xval_smb_noBFC10_outliers_genind$`Cross-Validation Results`) 
xval_smb_outlier_aves <- xval_smb_noBFC10_outliers_genind_df %>%
   group_by(n.pca) %>%
   summarize(mean_success = mean(success))

##25 PCs retained

#SMB neutral SNPs only
xval_smb_noBFC10_neutral_genind <- xvalDapc(smb_noBFC10_neutral_genind, smb_pops, n.pca.max = 300, training.set = 0.9,result = "groupMean", center = TRUE, scale = FALSE, n.pca = NULL, n.rep = 30, xval.plot = TRUE)

xval_smb_noBFC10_neutral_genind_df <- as.data.frame(xval_smb_noBFC10_neutral_genind$`Cross-Validation Results`) 
xval_smb_neutral_aves <- xval_smb_noBFC10_neutral_genind_df %>%
   group_by(n.pca) %>%
   summarize(mean_success = mean(success))

#5 PCs retained

#Neosho outlier SNPs only
xval_neosho_noBFC10_outliers_genind <- xvalDapc(neosho_noBFC10_outliers_genind, neosho_pops, n.pca.max = 300, training.set = 0.9,result = "groupMean", center = TRUE, scale = FALSE, n.pca = NULL, n.rep = 30, xval.plot = TRUE)

xval_neosho_noBFC10_outliers_genind_df <- as.data.frame(xval_neosho_noBFC10_outliers_genind$`Cross-Validation Results`) 
xval_neosho_outlier_aves <- xval_neosho_noBFC10_outliers_genind_df %>%
   group_by(n.pca) %>%
   summarize(mean_success = mean(success))

#6 PCs retained

#Neosho neutral SNPs only
xval_neosho_noBFC10_neutral_genind <- xvalDapc(neosho_noBFC10_neutral_genind, neosho_pops, n.pca.max = 300, training.set = 0.9,result = "groupMean", center = TRUE, scale = FALSE, n.pca = NULL, n.rep = 30, xval.plot = TRUE)

xval_neosho_noBFC10_neutral_genind_df <- as.data.frame(xval_neosho_noBFC10_neutral_genind$`Cross-Validation Results`) 
xval_neosho_neutral_aves <- xval_neosho_noBFC10_neutral_genind_df  %>%
   group_by(n.pca) %>%
   summarize(mean_success = mean(success))
...
vcf_midark_white_elk_neosho_outliers <- read.vcfR("../../raw_data/dapc_data/neosho_snps/MIDARK_WHITE_ELK_neosho_outlier_snps.vcf")
vcf_midark_white_elk_neosho_outliers_genind <- vcfR2genind(vcf_midark_white_elk_neosho_outliers)
vcf_midark_white_elk_neosho_outliers_genind@pop <- as.factor(midark_white_elk_pops)

vcf_midark_white_elk_neosho_outliers_genind <- scaleGen(vcf_midark_white_elk_neosho_outliers_genind, 
                                           center = TRUE, 
                                           scale = TRUE, 
                                           NA.method = c("mean"), 
                                           truenames = TRUE)

vcf_midark_white_bayou_neosho_outliers <- read.vcfR("../../raw_data/dapc_data/neosho_snps/MIDARK_WHITE_BAYOU_neosho_outlier_snps.vcf")
vcf_midark_white_bayou_neosho_outliers_genind <- vcfR2genind(vcf_midark_white_bayou_neosho_outliers)
vcf_midark_white_bayou_neosho_outliers_genind@pop <- as.factor(midark_white_bayou_pops)

vcf_midark_white_bayou_neosho_outliers_genind <- scaleGen(vcf_midark_white_bayou_neosho_outliers_genind, 
                                           center = TRUE, 
                                           scale = TRUE, 
                                           NA.method = c("mean"), 
                                           truenames = TRUE)

vcf_midark_white_uppark_neosho_outliers <- read.vcfR("../../raw_data/dapc_data/neosho_snps/MIDARK_WHITE_UPPARK_neosho_outlier_snps.vcf")
vcf_midark_white_uppark_neosho_outliers_genind <- vcfR2genind(vcf_midark_white_uppark_neosho_outliers)
vcf_midark_white_uppark_neosho_outliers_genind@pop <- as.factor(midark_white_uppark_pops)

vcf_midark_white_uppark_neosho_outliers_genind <- scaleGen(vcf_midark_white_uppark_neosho_outliers_genind, 
                                           center = TRUE, 
                                           scale = TRUE, 
                                           NA.method = c("mean"), 
                                           truenames = TRUE)

vcf_midark_skia_illi_neosho_outliers <- read.vcfR("../../raw_data/dapc_data/neosho_snps/MIDARK_SKIA_ILLI_neosho_outlier_snps.vcf")
vcf_midark_skia_illi_neosho_outliers_genind <- vcfR2genind(vcf_midark_skia_illi_neosho_outliers)
vcf_midark_skia_illi_neosho_outliers_genind@pop <- as.factor(midark_skia_illi_pops)

vcf_midark_skia_illi_neosho_outliers_genind <- scaleGen(vcf_midark_skia_illi_neosho_outliers_genind, 
                                           center = TRUE, 
                                           scale = TRUE, 
                                           NA.method = c("mean"), 
                                           truenames = TRUE)
...
scatter(dapc_neosho_neutrals)

    #PCs used: 10
    #Discriminant Functions retained:2


dapc_midark_white_elk_neosho_outliers <- dapc(vcf_midark_white_elk_neosho_outliers_genind, midark_white_elk_pops)

dapc_midark_white_bayou_neosho_outliers <- dapc(vcf_midark_white_bayou_neosho_outliers_genind, midark_white_bayou_pops) 

scatter(dapc_midark_white_bayou_neosho_outliers)

dapc_midark_white_uppark_neosho_outliers <- dapc(vcf_midark_white_uppark_neosho_outliers_genind, midark_white_uppark_pops)  
scatter(dapc_midark_white_uppark_neosho_outliers)

dapc_midark_skia_illi_neosho_outliers <- dapc(vcf_midark_skia_illi_neosho_outliers_genind, midark_skia_illi_pops)    
scatter(dapc_midark_skia_illi_neosho_outliers)

    
##Get the little PCA-explained and LDs used graphs from the dapc analyses
pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Genomics/visualization/outlier_fst_figures/dapc_smb_outlier_pca_da.pdf", width=11, height=9)

scatter(dapc_smb_outliers, posi.da = "topright", scree.pca = TRUE, posi.pca = "bottomright")

dev.off()

pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Genomics/visualization/outlier_fst_figures/dapc_smb_neutral_pca_da.pdf", width=11, height=9)

scatter(dapc_smb_neutrals, posi.da = "bottomright", scree.pca = TRUE, posi.pca = "topright")

dev.off()

pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Genomics/visualization/outlier_fst_figures/dapc_neosho_outliers_pca_da.pdf", width=11, height=9)

scatter(dapc_neosho_outliers, posi.da = "bottomright", scree.pca = TRUE, posi.pca = "topright")

dev.off()

pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Genomics/visualization/outlier_fst_figures/dapc_neosho_neutrals_pca_da.pdf", width=11, height=9)

scatter(dapc_neosho_neutrals, posi.da = "topright", scree.pca = TRUE, posi.pca = "bottomright")
...
pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Genomics/visualization/outlier_fst_figures/dapc_plots_lineages_more_compact.pdf", width=11, height=9)
...
## Plot Results for Xval cross-validaiton 
```{r}
#SMB outlier SNPs only
xval_smb_outlier_aves <- xval_smb_noBFC10_outliers_genind_df %>%
   group_by(n.pca) %>%
...
xval_smb_outlier_sd <- xval_smb_noBFC10_outliers_genind_df %>%
   group_by(n.pca) %>%
   summarize(sd_success = sd(success))

xval_smb_outlier_data <- merge(xval_smb_outlier_aves, xval_smb_outlier_sd, by = "n.pca")

xval_smb_outlier_plot <- ggplot(xval_smb_outlier_data, aes(x = n.pca, y = mean_success)) +
   ...
   theme(axis.text = element_text(size = 20)) +
   theme(axis.title = element_text(size = 20)) +
   scale_x_continuous("PCs retained", labels = as.character(xval_smb_outlier_data$n.pca), breaks = xval_smb_outlier_data$n.pca) +
   theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
...
xval_smb_neutral_sd <- xval_smb_noBFC10_neutral_genind_df %>%
   group_by(n.pca) %>%
   summarize(sd_success = sd(success))

xval_smb_neutral_data <- merge(xval_smb_neutral_aves, xval_smb_neutral_sd, by = "n.pca")

xval_smb_neutral_plot <- ggplot(xval_smb_neutral_data, aes(x = n.pca, y = mean_success)) +
   geom_errorbar(aes(ymin = mean_success - sd_success, ymax = mean_success + sd_success), width=2) +
   geom_point(fill = "grey", color = "black", pch = 21, size = 6, show.legend = F) +
   theme_set(theme_cowplot(12)) +
   labs(x = "PCs retained", y = "Average CV success (%)") +
   theme(axis.text = element_text(size = 20)) +
   theme(axis.title = element_text(size = 20)) +
   scale_x_continuous("PCs retained", labels = as.character(xval_smb_neutral_data$n.pca), breaks = xval_smb_neutral_data$n.pca) +
   theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
...
xval_neosho_outlier_sd <- xval_neosho_noBFC10_outliers_genind_df %>%
   group_by(n.pca) %>%
   summarize(sd_success = sd(success))

xval_neosho_outlier_data <- merge(xval_neosho_outlier_aves, xval_neosho_outlier_sd, by = "n.pca")

xval_neosho_outlier_plot <- ggplot(xval_neosho_outlier_data, aes(x = n.pca, y = mean_success)) +
   geom_errorbar(aes(ymin = mean_success - sd_success, ymax = mean_success + sd_success), width=2) +
   geom_point(fill = "grey", color = "black", pch = 21, size = 6, show.legend = F) +
   theme_set(theme_cowplot(12)) +
   labs(x = "PCs retained", y = "Average CV success (%)") +
   theme(axis.text = element_text(size = 20)) +
   theme(axis.title = element_text(size = 20)) +
   scale_x_continuous("PCs retained", labels = as.character(xval_neosho_outlier_data$n.pca), breaks = xval_neosho_outlier_data$n.pca) +
   theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
...
xval_neosho_neutral_sd <- xval_neosho_noBFC10_neutral_genind_df %>%
   group_by(n.pca) %>%
   summarize(sd_success = sd(success))

xval_neosho_neutral_data <- merge(xval_neosho_neutral_aves, xval_neosho_neutral_sd, by = "n.pca")

xval_neosho_neutral_plot <- ggplot(xval_neosho_neutral_data, aes(x = n.pca, y = mean_success)) +
   geom_errorbar(aes(ymin = mean_success - sd_success, ymax = mean_success + sd_success), width=2) +
   geom_point(fill = "grey", color = "black", pch = 21, size = 6, show.legend = F) +
   theme_set(theme_cowplot(12)) +
   labs(x = "PCs retained", y = "Average CV success (%)") +
   theme(axis.text = element_text(size = 20)) +
   theme(axis.title = element_text(size = 20)) +
   scale_x_continuous("PCs retained", labels = as.character(xval_neosho_neutral_data$n.pca), breaks = xval_neosho_neutral_data$n.pca) +
   theme(panel.border = element_rect(colour = "black", fill=NA, size=1))


pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Genomics/visualization/outlier_fst_figures/dapc_xval_plots.pdf", width=13, height=14)

plot_grid(xval_smb_outlier_plot, xval_smb_neutral_plot, xval_neosho_outlier_plot, xval_neosho_neutral_plot, nrows = 2, ncol = 2)
