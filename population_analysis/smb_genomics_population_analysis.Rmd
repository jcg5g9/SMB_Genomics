...
load("../admixture_phylogenomics_analysis/data/filtering_data/bcf10.Rda")
...
```{r}
# Load in the .Rda file generated in the admixture analysis
load("../admixture_phylogenomics_analysis/data/filtering_data/admixed_individuals.Rda")
...
```{r}
# Load in the .Rda file generated in the admixture analysis
load("../admixture_phylogenomics_analysis/data/filtering_data/pure_individuals.Rda")
...
# Modulate map state
smb_neosho_mapstate <- extractValue(smb_neosho_treexml,"Pop") # map state as a finestructure clustering

smb_neosho_mapstatelist <- popAsList(smb_neosho_mapstate) # .. and as a list of individuals in populations

smb_neosho_popnames <- lapply(smb_neosho_mapstatelist, NameSummary) # population names IN A REVERSIBLE FORMAT (I.E LOSSLESS)
...
smb_neosho_popnamesplot <-lapply(smb_neosho_mapstatelist, NameMoreSummary) # a nicer summary of the populations
...
# Modulate map state
pure_mapstate <- extractValue(pure_treexml,"Pop") # map state as a finestructure clustering

pure_mapstatelist <- popAsList(pure_mapstate) # .. and as a list of individuals in populations

pure_popnames <- lapply(pure_mapstatelist, NameSummary) # population names IN A REVERSIBLE FORMAT (I.E LOSSLESS)
## NOTE: if your population labels don't correspond to the format we used (NAME<number>) YOU MAY HAVE TROUBLE HERE. YOU MAY NEED TO RENAME THEM INTO THIS FORM AND DEFINE YOUR POPULATION NAMES IN popnamesplot BELOW

pure_popnamesplot <-lapply(pure_mapstatelist, NameMoreSummary) # a nicer summary of the populations
...
# Modulate map state
admixed_mapstate <- extractValue(admixed_treexml,"Pop") # map state as a finestructure clustering

admixed_mapstatelist <- popAsList(admixed_mapstate) # .. and as a list of individuals in populations

admixed_popnames <- lapply(admixed_mapstatelist, NameSummary) # population names IN A REVERSIBLE FORMAT (I.E LOSSLESS)
## NOTE: if your population labels don't correspond to the format we used (NAME<number>) YOU MAY HAVE TROUBLE HERE. YOU MAY NEED TO RENAME THEM INTO THIS FORM AND DEFINE YOUR POPULATION NAMES IN popnamesplot BELOW

admixed_popnamesplot <-lapply(admixed_mapstatelist, NameMoreSummary) # a nicer summary of the populations
...
metadata_populations <- metadata_populations  %>% 
  mutate(sample_id = factor(sample_id), 
         population = factor(sample_id))

# Save the new .map file
save(metadata_populations, file = "data/metadata/metadata_population.Rda")
```