...
### STEP 1: Filter out Spotted Bass individuals and generate pure sample and admixed sample datasets for the full, non-thinned VCF.
...
##### 1b.1. Read in metadata and generate a list of only the Spotted Bass hybrid; run the Rmd chunk below, which is borrowed from `smb_genomics_admixture_analysis.Rmd`, lines 321-341:

##### Generate list of Spotted Bass hybrid: `data/filtering_data/bfc10.txt` 
```{r}
# Load in the metadata .Rda file
...
##### 3b.2. Read in metadata and generate a list of only Spotted Bass to be used for filtering in subsequent steps:
